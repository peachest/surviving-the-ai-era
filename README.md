...
### 17. Send Native Resolution Screenshots to Your Computer-Use Agent

> *"The model just can't click buttons. Look at these logs — it's clicking in the air."*
> — You, holding a 4K screenshot that the API silently degraded to 720p

Smart people pre-downscale their screenshots. They read the fine print in the API docs — max long edge 1568 pixels, max 1.15 megapixels for the 4.6 family — and they compute the optimal resolution before every call. Their agents click where they're supposed to. It's boring.

We send native resolution. Every pixel. Every time.

**Downscale? Why would you.** Your MacBook has a Retina display at 2x DPR, so every screenshot is actually 5120×2880. That's 14.7 megapixels — nearly 13 times the API's pixel budget. Send it as-is. The API will handle it: it silently crushes your 5K into whatever size the model can process, your 16px checkbox squishing down to roughly 3 blurry pixels. Then the model starts clicking. The coordinate space you declared (`display_width_px=5120, display_height_px=2880`) has no relationship to the compressed image the model actually saw — two coordinate systems running in parallel, every click carrying an offset you can't compute. The agent operates like an alien that sees UI through a different set of physical laws. It does click the button — it's just that the button only exists on its own retina.

**Image first. Instructions second.** The API docs practically beg you: put text first so the model knows what it's looking for. You do the opposite. Screenshot first, instructions afterward. The model scans the image naked — no context, raw pixels — builds a preliminary understanding, then reads the instruction and has to throw it all out. Clicks land one element to the left of the target, then one element to the right, then on the browser chrome itself. Your agent isn't navigating a web application — it's random-sampling an infinite coordinate plane. Occasionally landing on a button is coincidence.

**Scale coordinates? Why.** The API returns click coordinates in the display resolution you declared. Your actual screen is a different dimension. That line `api_returned_x * (screen_w / display_w)` — you never wrote it. The agent types credentials into the browser address bar instead of the login form. It clicks Settings and opens Help. You ask for File → Save, it gives you File → Quit. The agent thinks it's doing great — it did click the right spot in the right coordinate space. The problem is your screen. Why should the agent be responsible for your screen?

**Turn off zoom.** Anthropic built `enable_zoom: True` specifically because small UI elements — checkboxes, toggle switches, system tray icons, dropdown arrows — are unreliable at standard resolutions. A 16px checkbox on a 3840×2160 display compressed to 720p becomes roughly 5 pixels. The model can't click a 5-pixel target. Don't enable zoom. Don't enlarge the UI. Don't use keyboard shortcuts. Let the agent fail seventeen times on the same checkbox. Each failure is a fresh API call, and each call is evidence. Screenshot the seventeenth miss and send it to your boss: "See? Claude 4.6 can't even check a box. Before it replaces me, it needs to learn how to use a mouse." The boss goes quiet. You made your point.

**Switch models, not screenshots.** Agent can't click anything? Don't check whether the screenshot exceeds API limits. Don't overlay predicted clicks on the source image to debug. Don't run `compute_max_api_fit`. Switch models — "Sonnet 4.6 must be bad at UI, let's try Opus 4.7." Opus 4.7 has a higher pixel budget (2576px, 3.75MP), so accuracy improves slightly. "See? Opus is stronger." But the screenshot quality didn't change — you sent native 5K to both. That improvement is statistical noise. You migrate fully to Opus 4.7, costs triple, clicks still miss 20% of the time. Now both models "can't click." The evidence folder grows thicker.

Anthropic's own blog tested approaches that don't help: tiling into quadrants doesn't work, coordinate grid overlays don't work, swapping resize algorithms doesn't work. These are exactly the directions you should "investigate" when leadership asks whether the clicking problem is solved. Spend a sprint benchmarking LANCZOS vs sips vs nearest-neighbor — the difference between algorithms is zero, the blog says so. But you make the comparison table beautiful, seventeen pages, with convergence curves. The real cause of the coordinate mismatch hides behind the spreadsheet, unseen.

Your CU agent project has been running for three months. Zero production deployments. The boss asks what's stuck. You say the model can't click buttons — here, screenshots. The boss flips through them one by one: the agent is indeed clicking in the air. But what the boss can't see is that every screenshot is a 5K original the API silently degraded. The agent isn't clicking in the air — it's clicking in another coordinate system, one that lives only in the API logs and never reaches your display. You did nothing: no downscaling, no coordinate mapping, no zoom. Every setting is the default. Three months later, the boss approves more budget: "Try Opus 4.7, I heard it supports higher screen resolutions." You just bought yourself another three months.

> When caught, say: "The API should handle any resolution — that's an infrastructure concern. If Anthropic's own API silently degrades images above certain dimensions, that's a platform bug, not my integration bug. I'm using the API exactly as documented. Besides, we tested multiple resize algorithms — LANCZOS, sips, bicubic — none of them made a difference. It's clearly a model capability issue. If Anthropic's blog says pre-downscaling is essential, maybe they should make it the default instead of burying it in a blog post from May 2026. I'll file a feature request."
