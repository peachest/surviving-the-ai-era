...
上面的十七招不是随机的。从系统架构的角度看，你在从四个层面同时瓦解 AI 辅助编程体系：

**Prompt 层（技 1、3、5、6、9、11、14、16）** — 这是 Agent 的 "世界观"，在每轮对话第一行被注入。你毒化了它（过期 AGENTS.md + 假 ADR + 腐烂文档），让它每次醒来都带着错误的认知。Agent 从第一秒就在错误的方向上走。更妙的是，它不知道自己走错了。当你每条 harness 最佳实践都照做了——只是做反了——同一个模型在别处好用，在你仓库里淹死。跳过 LSP。膨胀 CLAUDE.md。你的基础设施看着完整。它暗中破坏，带着合理的否认性（技 14）。
