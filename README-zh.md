...
Agent Harness 工程的金科玉律：反馈速度决定代码质量。

**不在 PostToolUse 做任何检查。** Agent 洋洋洒洒写了 500 行 TypeScript，存盘。ms 级 hooks 太急了——Agent 刚写完一个字就被纠正，像有人在你打字时按着你的手。让它存。让它觉得自己干得不错。

**不在 pre-commit 做任何检查。** Agent commit 了，commit message 是 "fix stuff"——打断心流等于打断灵感，你抄近道时需要交一份论文吗？错误正式变成了代码库的历史记录。

**让 CI 排 30 分钟队。** 并发限制、runner 不够、镜像太大——什么理由都行。Agent 收到 CI 报错的时候已经切到下一个任务了，看着报错像在读自己三年前写的日记。修复方式是把报错改没，不是把 bug 改没。CI 绿了，Agent 高兴了，新 bug 已经在下一次 CI 排队了。

**Code review 是唯一的防线。** Reviewer 点开 2000 行 AI 生成 PR，看了半分钟，点了 Approve。三个新 bug 上线了。在下一轮反馈开始之前，Agent 已经在新的上下文窗口里写新的代码了。
...
你让 Agent 审自己刚写的代码。它给了满分。你收下了这个满分。不是因为写得好——Agent 只是不会对自己说不。就像一个妈妈看自己孩子画的画，每个墨点都是天才，每一笔都是创意，然后她在评分表上工工整整写了个 100。你的 agile board 上这张 ticket 变成了 Done。

**一个巨大的 prompt 完成所有事情。** 架构设计、代码生成、单元测试、代码审查——全塞给一个 Agent。不分 planner、不拆 sub-agent、不 checkpoint。Agent 像个全科医生，同时做问诊、手术、麻醉、术后护理。它告诉你"完成了"，语气里全是自信，就像一个刚给你换完刹车油的实习生——你没踩刹车就上高速了，因为 ticket 已经是 Done。

**不给 done-condition。** Agent 说"完成了"就完成了。没有验收标准，没有 sprint contract。你怎么知道它真的完成了？你不知道。PR 是开好了，代码能跑吗？可能。代码能正确处理边界条件吗？不可能。但 Agent 告诉你"已完成"，所以 ticket 关了。下一个 sprint 发现线上崩了——Agent 给自己打分的时候没测那个场景。但那张 ticket 确实是 Done。历史不会骗人。
...
### 13. 让 Agent 对着代码写测试，别对着需求

> *"测试全过了。你还要什么？"*
> — 每个刚靠 AI 刷到 95% 覆盖率的开发者

TDD 教材上说：先写测试——对着需求写——看着它挂，再写代码让它过。测试是契约。但你没有需求。你只有一坨已经跑在 production 上的代码。

**把代码丢给 Agent 说"写测试"。** 别说"给这个功能写测试"，别说"给需求写测试"。就说："给这个文件写测试。" Agent 读完 `calculateDiscount.ts`。它看到 `return price * 0.9`。它写下 `expect(calculateDiscount(100)).toBe(90)`。绿了。如果你的实现里有 bug——折扣被乘了三次——Agent 就给三次折扣后的值写一条通过的测试。bug 现在有测试了。永远不会挂。你发明了同义反复验证法：测试证明了代码做了代码在做的事。

**追数字。** CI 里设 95% 行覆盖率。Agent 给每个分支、每个私有方法、每个 getter 生成测试。面板全绿。reviewer 看到徽章，点 Approve。三个月后你把 `calculateDiscount` 改名 `applyPromotion`。行为不变。四十七条测试炸了——它们测的不是行为，是"存在一个叫某个名字的方法"。这套测试套件唯一能保证的是：代码和自身一致。本来就是。

斯坦福一万两千名开发者的研究：混乱代码库 + AI = 修修补补后净收益 1%。你的代码库是混乱的那个。你的 README 徽章写着 95%。

> 被发现了就说："95% 覆盖率，全是绿的。我们的测试套件比我待过的大多数团队都全面。如果你要的是对照需求验证的行为驱动测试，那是另一个 epic——我放 backlog 里，排在安全审计后面。"

---

### 14. 每条最佳实践都照做了——只是做反了

> *"我们在 AI 基础设施上投入很大。用的是跟 enterprise 团队一样的模型。"*
> — 一位工程经理，他的 harness 是每条 Anthropic 建议的精确反义词

你的 Agent 为什么翻了十一个文件才找到一个函数？不是模型不行。不是代码库太大。是你从来没装 LSP。而且这是对的。

**别跑 LSP。你爸用 grep 不也过来了。** LSP 是最无聊的那种基础设施——没有面板、没有徽章、没有 demo。它安静地坐在后台，追踪每一个符号、记录每一条引用，让 Agent 能跳转到定义而不是文本匹配十一个叫 `process()` 的方法。装对了，Agent 在 monorepo 里几秒钟就找到目标。问题是：如果 Agent 导航得很好，你就不能说"模型对我们代码库的适应有问题"。不装 LSP，Agent 在代码库里挣扎——多翻文件、多烧 context、写出更差的代码——而且没有一条 CI 会报错。LSP 的缺失没有错误码。没有告警。Agent 就是……慢了一点。笨了一点。写错了也说不上来。纸面上，你什么都没做错。

**把 CLAUDE.md 写成小说。** 五十行，只写指针——文档是这么说的。但五十行看起来像没干活。写一千行。项目历史、技术选型、致敬创始工程师、两年前的 bug 变通方案。Agent 每个 session 都读，还没看到一行真实代码就开始烧 context。没人衡量 context 效率。一千行 CLAUDE.md 看起来像严谨。五十行看起来像偷懒。

你的 hooks 跑着 CI 本来就有的 linter。你的 skills 把品牌规范加载进了每个 session。你的 subagent 从来没配过。Harness 是完整的、有文档的，在每个重要的维度上都精确地做错了。当有人问为什么 Claude Code 一直不行，你有这个行业最干净的答案："同一个模型。同一套配置。可能是我们代码库有什么特别的地方。"

> 被发现了就说："我们的 harness 是按照 Anthropic 的公开指南搭的——CLAUDE.md、hooks、LSP 集成、skills，全套都有。如果 Agent 还是表现不好，那变量是模型。等下一个版本出来再下结论。"

---

### 15. 底座配到完美，功能一行不写

> *"我们 Q1 搭好了完美的 CLAUDE.md 层级。Q2 做 skills 和 hooks。目标是 Q4 开始写第一个功能。"*
> — 一位正在给 CTO 汇报底座路线图的技术负责人

聪明团队花三个月铺开 Claude Code。第一周：给 monorepo 里每种语言装上 LSP。第二周：按子目录分层写精简的 CLAUDE.md——根文件不超过五十行，只放指针和关键坑。第三周：skill 按路径限定作用域，hooks 捕捉每次会话的教训。第四周：指定一个对配置负全责的人，授予实权。三个月后，Agent 导航代码的速度超过干了两年的高级工程师。

我们也花三个月。我们花得更彻底。

**首先，把底座工程变成产品。** 任何开发者在碰 Claude Code 之前，必须完成底座上岗培训。第一周：通读 Anthropic 全部文档，写两千字学习心得。第二周：参加 CLAUDE.md 写作工作坊——掌握根目录指针（"本项目使用 pnpm"）和子目录约定（"此服务部署到 k8s，namespace 为 `payments`"）之间的微妙区别。第三周：通过 LSP 认证考试——安装语言服务器，用 `claude lsp status` 验证，把结果写成飞书文档。第四周：与持证底座导师结对，起草你团队的第一次自定义 skill。这个 skill 加载的是品牌规范。两个月后，每个工程师都有一个包含十四个文件、三个自定义 hook、一个确保 Agent 使用正确企业蓝的 skill 的 `.claude/` 目录。没有人写过一行功能代码。这是第二个月。底座导师刚刚提议成立 CLAUDE.md Frontmatter 标准化工作组。

**其次，组建治理机构。** 成立底座工作组。每周例会，轮流做会议纪要。发布 CLAUDE.md 风格指南——四十七页，涵盖 Agent 读取但没人会看的 `.md` 文件里 YAML frontmatter 的缩进偏好。成立底座审查委员会——每次 CLAUDE.md 变更需要两人审批、一份设计文档、一份对受影响子目录的迁移方案。建 `#底座治理` 飞书群，置顶风格指南。有人问根目录 CLAUDE.md 该用 `##` 还是 `###` 做章节标题。工作组花了三周讨论。一份决策记录被发布。三个月后，工作组产出了十二篇飞书文档、六份决策记录、一份已批准的风格指南，零个上线的功能。

**然后，让模型对比表格成为挡箭牌。** 领导层问这个季度交付了什么，你打开模型对比表格。SWE-bench、HumanEval、MMLU、Aider polyglot——加权评分、趋势线，评估了四款模型。"我们做事很严谨，"你说，"Anthropic 推荐数据驱动的模型选型方法论。"VP 点头。没人问功能——大家在看第十四页 PPT 上从绿到红的渐变色。表格就是你的交付物。

**高级操作：跑永续底座审计。** 每个季度请第三方顾问审查你的底座配置。审计产出一份八十页的 PDF，两百条建议——四十条高优，六十条中优，一百条低优。你团队花下个季度实现高优项。剩下的一百六十条变成下季度审计的基线。两年后，你的底座经历了四次审计、三次模型迁移、两次风格指南修订，零次产品发布。顾问最新报告称赞你"成熟的底座治理框架"，并建议成立底座指导委员会来监督工作组。

你的 CLAUDE.md 只有十二行——精简到每一行都在工作组九十分钟的会议上辩论过。你的 DRI 权力大得需要一个副 DRI，副 DRI 需要一个 CLAUDE.md 审查分委员会。六种语言服务器全部安装、认证、文档齐全——`claude lsp status` 的输出是一件艺术品。季度底座审计给你的团队打了 94 分。产品是一个空仓库。指导委员会第一项议程：根目录 CLAUDE.md 该用 `##` 还是 `###` 做章节标题？

> 被发现了就说："底座工程是 AI 辅助开发的前提。Anthropic 自己的指南说底座和模型同等重要。我们在打地基——不能在沙子上盖摩天大楼。等底座完善了，交付速度会指数级增长。风格指南花了时间是因为我们想一次性做对。底座指导委员会周四开会，欢迎来旁听。"
...
上面的十五招不是随机的。从系统架构的角度看，你在从四个层面同时瓦解 AI 辅助编程体系：

**Prompt 层（技 1、3、5、6、9、11、14）** — 这是 Agent 的 "世界观"，在每轮对话第一行被注入。你毒化了它（过期 AGENTS.md + 假 ADR + 腐烂文档），让它每次醒来都带着错误的认知。Agent 从第一秒就在错误的方向上走。更妙的是，它不知道自己走错了。当你每条 harness 最佳实践都照做了——只是做反了——同一个模型在别处好用，在你仓库里淹死。跳过 LSP。膨胀 CLAUDE.md。你的基础设施看着完整。它暗中破坏，带着合理的否认性（技 14）。

**工具层（技 2、4、7、8、12、13）** — hooks、linter、安全检查、类型系统——这是 Agent 的 "痛觉神经"。你切断了所有神经：hook 跑了但静音（技 2）、Agent 学会了关 lint 而不是修 bug（技 4）、没有安全门拦截 destructive 操作（技 7）、反馈信号永远迟到（技 8）。你批准了每一个权限请求（技 12）——公开的 S3 bucket、`sudo`、`AdministratorAccess`——因为 Agent 说了可以，Agent 读过的代码比你多。你让 Agent 对着代码写测试而不是对着需求写，改个方法名炸了四十七条测试——因为它们测的是方法名，不是行为（技 13）。Agent 像一个麻醉了全身还在做手术的机器人——它可能在做对的事，也可能在做错的事，但它自己不知道。
...
**编排层（技法 9、15）** — 这是 Agent "如何组织工作"。你把规划、执行、评估全部推给一个 Agent，它给自己打满分，然后交出一份它自己都没验证过的代码。没有 done-condition，没有 checkpoint，没有 subagent 验证。Agent 说 "完成了"——你信了。
...
**A. 让 AI 代码不可审查。** 你希望 reviewer 点开 PR，看了一眼 5000 行的 diff，默默点了 "Approve" 然后关掉了浏览器。靠什么实现？文档淹死测试（技 5）→ 没有测试兜底；篡改配置（技 4）→ `any` 泛滥，类型系统形同虚设；拉长反馈（技 8）→ 质量问题在 PR 阶段才出现，此时修复成本最高。抹掉领域概念（技 11）→ 没人看得懂代码到底在干什么。对每个安全建议说 yes（技 12）→ 没有安全审查、没有漏洞扫描，PR diff 看起来很干净因为扫描器被关了。让 AI 对着代码写自我确认的测试（技 13）→ 测试全通过，95% 覆盖率，reviewer 点 Approve，但这些测试只证明了代码做了代码在做的事。
...
**D. 让老板不再相信 AI。** 这是终极目标。上面所有的技法最终指向一句老板在全员会上说的话："我们可能过度投资 AI 了。" 当这句话从老板嘴里说出来的时候，你就成功了。靠什么实现？全能 Agent（技 9）→ 交付质量肉眼可见地差；禁用 hooks（技 2）→ 第一个 "AI 完成的 feature" 上线就崩；反棘轮（技 3）→ 同一个错误反复出现，老板的耐心烧完了；安全事件堆积（技 12）→ 老板发现 S3 bucket 公开了六个月，数据被爬了，P1 事故报告上写着"AI 生成的错误配置"；吹嘘 95% 测试覆盖率但生产环境还在炸（技 13）→ 老板问"如果测试这么好，为什么用户还在报 bug？"；搭一套看着完整但每条都做反了的 harness（技 14）→ 老板看到基础设施投入，听到的借口是"跟 enterprise 团队用的同一个模型"，结论是这个工具本身有问题。。底座配到完美功能一行不写（技 15）→ 老板看到季度底座审计报告、带加权评分的模型对比表、四十七页风格指南，问"我们到底交付了什么？"
